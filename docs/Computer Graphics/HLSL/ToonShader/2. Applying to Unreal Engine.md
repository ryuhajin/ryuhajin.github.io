...
## Domain 속성
- Surface (표면)
  - 가장 기본값. 3D 모델의 표면에 칠하는 일반적인 재질입니다.
  - Base Color, Metallic, Roughness 등의 물리 기반 렌더링(PBR) 입력들을 가집니다.
- Post Process (포스트 프로세스)
  - 렌더링이 완료된 최종 2D 화면 이미지에 적용하는 필터 용도
  - 입력으로 SceneTexture 노드를 통해 화면 색상, 깊이, 노멀 등의 데이터를 가져올 수 있음
  - 최종 결과물은 Emissive Color로 출력합니다.
- UI
  - UMG(Unreal Motion Graphics) 위젯에 사용되는 2D 인터페이스용 머티리얼입니다.
- Light Function (라이트 함수)
  - 광원의 빛에 패턴이나 마스크를 적용하여 그림자를 만들거나(예: 창살 그림자), 프로젝터처럼 이미지를 투사하는 효과를 낼 때 사용
- Deferred Decal (디퍼드 데칼)
  - 벽에 붙은 포스터, 핏자국, 총알 자국처럼 다른 표면 위에 추가로 이미지를 덧씌울 때 사용합니다.
