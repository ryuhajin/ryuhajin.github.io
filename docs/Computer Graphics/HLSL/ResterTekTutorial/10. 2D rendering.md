...
## Dynamic Vertex Buffers in 2D rendering
 ...
 Dynamic Vertex Buffers란?

  Dynamic Vertex Buffer(동적 정점 버퍼)는 내용이 자주 변경될 것으로 예상되는 정점 데이터를 저장하기 위해
  특별히 설계된 GPU 메모리 영역입니다.

  핵심 아이디어는 CPU에서 매 프레임 또는 주기적으로 계산되는 새로운 정점 데이터를 최소한의 비용으로 GPU에
  빠르고 효율적으로 전송하는 것입니다.

---

2D 렌더링에서 동적 정점 버퍼를 사용하는 이유는 2D 객체들의 특성과 렌더링 최적화 기법인 '배치(Batch)
  렌더링' 때문입니다.

   1. 끊임없이 변하는 2D 객체:
       * UI 요소: 마우스 커서, 클릭에 반응하는 버튼, 움직이는 창, 바뀌는 텍스트 등은 매 프레임 위치나 모양,
         내용이 바뀔 수 있습니다.
       * 스프라이트: 캐릭터나 적들은 계속해서 위치를 바꾸고, 애니메이션에 따라 텍스처 좌표(UV)가 바뀔 수
         있습니다.

      이처럼 매 프레임 정점 데이터(위치, 텍스처 좌표 등)가 바뀌는 객체들을 효율적으로 처리하려면, 매번
  버퍼를 새로 생성하는 것보다 동적 버퍼의 내용을 Map/Unmap으로 업데이트하는 것이 훨씬 빠릅니다.

   2. 배치 렌더링 (Batch Rendering) - 가장 중요한 이유:
      GPU에 그리기 명령(Draw Call)을 한 번 내리는 것은 상당한 비용이 드는 작업입니다. 만약 화면에 100개의
  스프라이트가 있다면, 100번의 Draw 명령을 내리는 것은 매우 비효율적입니다.

      배치 렌더링은 이 Draw 명령 횟수를 줄이기 위한 기법입니다.
       * 과정:
           1. 한 프레임에 그려야 할 모든 2D 객체(스프라이트, UI 등)의 정점 데이터를 CPU의 메모리(예: 배열)에
              모두 모읍니다.
           2. 이 데이터를 하나의 큰 동적 정점 버퍼에 Map/Unmap을 통해 한 번에 복사합니다.
           3. GPU에 단 한 번의 `Draw` 명령으로 버퍼에 있는 모든 정점을 그리라고 지시합니다.

      화면에 보이는 객체와 그 위치는 매 프레임 달라지므로, 이 모든 정점 데이터를 담는 '모음 버퍼'는 매
  프레임 내용이 완전히 새로 쓰여져야 합니다. 바로 이런 용도에 동적 정점 버퍼가 완벽하게 부합하는 것입니다.
  정적 버퍼로는 이런 작업을 효율적으로 수행할 수 없습니다.
...
- 만약 화면에 돌멩이 1,000개를 그려야 한다면, 가장 단순한 방법은 다음과 같습니다.
...
### 배치 렌더링 방식
- 배치 렌더링은 이 비효율을 개선
- 만약 1,000개의 돌멩이가 모두 같은 모양과 재질(텍스처, 셰이더)을 사용한다면, 굳이 1,000번씩이나 명령을
  내릴 필요가 없음
