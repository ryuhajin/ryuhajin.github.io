...
## 프레임 워크 요소
- **FontClass** 추가 : 글꼴 데이터를 저장하고 문자열 렌더링에 필요한 정점 버퍼 생성
...
> 순서대로 [문자의 ASCII 값] [문자] [왼쪽 텍스처 U 좌표] [오른쪽 텍스처 U 좌표] [문자의 픽셀 너비]
...
`Sample()` 함수는 HLSL에서 텍스처를 샘플링하기 위한 내장 함수
