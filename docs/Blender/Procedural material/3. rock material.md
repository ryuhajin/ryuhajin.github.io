...
## 노이즈로 좌표 비틀기
1. **mapping 노드의 vector 출력 (기본 텍스쳐 좌표 생성)**
    - 기본 좌표계 (구조/ 스케일 기준이 됨)
2. **noise texture fac, color 출력**
    - noise로 왜곡시키기
    - Fac : 단일 출력. 왜곡이 단조롭게 느껴질 수 있음
    - Color 출력 : 3축이 따로 놀 수 있어 왜곡이 더 자연스럽다
3. **1,2번을 Mix A,B (linear light)**
4. **해당 출력값을 voronoi texture 벡터 값으로 넣기**
    - 이미 왜곡된 좌표를 주기 때문에
    - 셀 경게가 늘어나고, 구겨지는 불규칙성이 생성됨

---

# Bump node
**높이(Height) 정보로 노멀을 만들어** 조명 반응을 바꾸는 노드

- 입력으로 들어오는 **Height(흑백 요철 정보) 를 이용**해 표면의 노멀을 기울어진 노멀로 바꿈
- **빛 계산에 쓰이는 노멀만 바꿔 그림자/하이라이트/스페큘러가 표면이 파인 것처럼 반응**하게 만든다
...
- 흰색 = 더 튀어나온 곳(높음)
- 검정 = 더 들어간 곳(낮음)
- 절차 노이즈(Fac), Voronoi Distance, 마스크 등을 그대로 넣어 요철 원본으로 씀

---

## Normal (vec)
**이 노멀을 기반으로 bump를 얹겠다**라는 기본 노멀 입력

- **비워두면 보통 메시의 기본 노멀(Geometry Normal) 을 사용**
- 여기에 Normal Map의 결과를 넣거나, 앞 단계 Bump의 Normal 출력을 넣어서 **‘레이어링’할 수 있다**
- Normal 입력은 normal map이나 다른 bump를 결합(체인)할 때 쓴다
...
- 0이면 변화 없음, **값이 커지면 최종 노멀에서 범프 노멀의 비중이 증가**
- **Strength ↑** = 범프 적용 비율 ↑
- **Strength ↓** = 범프 효과가 적용 비율 ↓

---

## Distance
Height는 보통 0~1 범위의 스칼라. **Distance는 이 값을 미터 단위의 높이로 해석하게 만드는 스케일**

- h_real = Height * Distance
- Distance가 커지면 높이 변화량이 커져서 노멀이 더 많이 꺾임
- **Distance ↑** = 범프 강도 ↑ (더 튀어나오고 더 깊어 보임)
- **Distance ↓** = 범프 강도 ↓ (완만/부드러움)
...
1. Distance = 범프의 실제 높이 스케일 (단위)
2. Strength = 범프 노멀을 적용하는 페이드(보간) 비율

Distance로 ‘얼마나 튀어나오게 할지’를 정하고, Strength는 마지막 미세 조절에 쓰는 편이 안정적
```
...
# output
## Normal (vec)
Bump가 만든 변형된 노멀 벡터가 출력

- Principled BSDF의 Normal 입력
- 또는 다른 Bump 노드의 Normal 입력에 연결

---

# Displacement node
Bump vs Displacement의 핵심 차이

- **Bump**: 표면 노멀만 속여서 울퉁불퉁해 보이게 함 (실루엣 변화 X)
- **Displacement**: **표면 기하 자체를 이동**시킴 (실루엣 변화 O, 진짜 파임/돌출)

> displacement는 실세 표면점을 이동시켜 반사를 바꾼다

---

## Displacement 개념
displacement 노드는 2가지 모드가 있다

### Height Displacement
**흑백 Height 값으로 표면 노멀 방향으로만 밀어냄**

```md
offset=(H−Midlevel)×Scale
```

- **H** : Height 입력 (보통 0~1)
- **Midlevel** : 0 변위 기준점
- **Scale** : 변위 강도 (얼마나 밀어낼지)

```md
H = Midlevel이면 변위 0

H > Midlevel이면 바깥으로 튀어나옴

H < Midlevel이면 안쪽으로 파임
```

> Midlevel을 맞추지 않으면 메시가 통째로 부풀어 오르거나 위치가 밀리는 현상이 흔함
> - Midlevel/Scale 조정을 신경쓰자

---

### Vector Displacement
height보다 고급스러운 형태. 단순히 노멀 방향이 아니라 **벡터 방향대로 표면을 이동**

- 오버행 (언더컷), 측면 변위 등 **노멀 기반으로는 불가능한 변형이 가능**

---

## Inputs
1. **Height (Value)**
    - 흑백 높이값. 노이즈/보로노이/마스크를 여기로 넣음
2. **Normal (Vector)**
    - 어느 노멀을 기준으로 밀어낼지의 기준 노멀
    - 보통 비워두지만, 특수한 노멀 체인 (예: 변형된 노멀 방향 기준)이 필요할 때 연결
3. **Midlevel (Value)**
    - 변위 0 기준점
    -   Height 텍스처가 0.5를 ‘평평’으로 만들어진 맵이면 보통 Midlevel=0.5로 맞춤 (이걸 안 맞추면 전체가 떠오르거나 꺼짐)
4. **Scale (Value)**
    - 변위 강도 (얼마나 밀어낼지)
    - 크면 돌출/함몰이 크다
...
1. **Displacement (Vector)**
    - 이 출력을 Material Output 노드의 Displacement 소켓에 연결해야 변위로 적용

---

## displacement가 적용이 안되는 경우
1. **Eevee는 셰이더 Displacement를 지원하지 않는 경우가 많음**
    - Eevee에서는 셰이더 기반 실제 displacement가 제한적
    - 이럴 땐 Displace Modifier 같은 방식이 필요
2. **메시가 너무 로우폴리면 움직일 점이 없음**
    - Displacement는 정점을 움직이는 것이라 정점 수가 적으면 디테일이 나오지 않음
    - Subdivision / Adaptive Subdivision (마이크로디스플)을 같이 쓰는 경우가 많다
3. **Cycles에서 `Bump Only`로 되어 있으면 실제 변위가 안 됨**
    - Cycles는 설정에 따라 진짜 displacement 대신 범프만 처리될 수 있음

> material output 노드 -> settings -> surface -> displacement를
> - bump olny에서 displacement and bump로 변경

---

# Node Group
Node Group은 **여러 개의 노드를 하나의 블록으로 캡슐화**해서, 그래프를 깔끔하게 만들고 재사용하기 위한 기능

- 바깥에서 보면 노드 하나
- 내부에 들어가면 노드 묶음(서브 그래프)

> UE의 머티리얼 함수와 동일하다고 보면 됨

---

## Node Group 만들기
## 생성
1. Shader Editor에서 노드 여러 개 선택
2. Ctrl + G → 그룹 생성
3. 선택한 노드들이 하나의 Node Group으로 묶임

## 그룹 내부로 들어가기 / 나오기
- 그룹 안으로 들어가기: 그룹 노드 선택 후 Tab 또는 더블클릭
- 그룹에서 나오기: Tab (상위로 나감)

## 입력/출력 추가
1. 그룹 내부로 이동 후 `N` 키로 사이드 패널 열기
2. 그룹 탭에서 그룹 소켓 추가, 이름 변경, 삭제 등 가능
