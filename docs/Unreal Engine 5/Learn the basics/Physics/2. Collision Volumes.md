...
# 3. Collision Events
스태틱 매시의 collision Presets으로 물체끼리 서로 충돌할 때 발생하는 이벤트 설정하기

- 스태틱 매시 디테일 - **collision** 카테고리에서 설정

1. hit 이벤트가 발생하려면 해당 액터에 대한 **simulation Hit Events 속성이 켜져있어야 함**
2. overlap 이벤트가 발생하려면 해당 액터에 대한 **overlap Event 속성이 켜져있어야 함**

> 또한 충돌에 관련된 두 액터 모두 켜져있어야 유효

---

## 새 프리셋 추가하기
1. 상단의 Edit -> Project Settings
2. Engine 카테고리의 collision
3. Object Channel에서 new Channel 생성 (예: enemies)
4. collision Presets의 object Type에 추가된 것 확인할 수 있음

---

## 충돌 이벤트 발생 시 실행시키기
두 가지 종류의 블루 프린트에서 충돌 이벤트 시 일어날 일을 설정할 수 있다
...
## 레벨 블루프린트
1. 레벨 이벤트 그래프 우클릭
2. 리스트의 Add Event for `스태틱 매시 이름` 카테고리의 collision
3. 콜리전 이벤트에 해당하는 노드 추가
4. 콜리전 이벤트 노드들이 참조하는 스태틱 메쉬 이름은 `()` 괄호 안에서 볼 수 있음

---

### 레벨 BP collision 노드
1. Add On Actor Begin Overlap
  - 다른 액터가 참조된 액터와 처음으로 겹치기 시작할 때 발동
  - Overlapped Actor : 자기 자신
  - Other Actor : 충돌과 관련된 다른 액터
2. Add On Actor End overlap
   - 다른 액터가 참조된 액터와 겹치는 것이 멈추자마자 실행
3. Add On Actor Hit
   - Self Actor : 자기 자신
   - Other Actor : 충돌과 관련된 다른 액터
...
## 액터 블루프린트
1. 블루 프린트 그래프 우클릭
2. collision 카테고리에서 콜리전 이벤트 노드 추가 가능

---

### 액터 BP collision 노드
레벨 콜리전 노드와 다른점 - 이름에 `on`이 붙지 않음, self Actor 핀 없음

1. Event Actor Begin Overlap
2. Event Actor end Overlap
3. Event Hit
