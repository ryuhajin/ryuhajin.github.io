...
> 핵심 : 재귀적으로 분열하는 큐브
> - Event BeginPlay는 액터 인스턴스(Actor Instance)가 월드에 스폰될 때 단 한 번만 실행

```
맨 처음 큐브(부모 큐브)가 BeginPlay를 실행 (1회)

이 부모 큐브가 SpawnActor 노드로 8개의 자식 큐브를 '새로' 스폰

이 새롭게 태어난 8개의 자식 큐브들은 각각이 독립된 '인스턴스'

따라서 이 8개의 자식 큐브 각각이 자신의 Event BeginPlay를 또 1회씩 실행
```
