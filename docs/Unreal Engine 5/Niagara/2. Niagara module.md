...
# Event
프로퍼티 옆 `+stage`를 통해 이벤트 추가

> 프로퍼티에서 퍼시스턴스 ID 필요 체크!

## Event handler
이미터 통신

- 통신할 이미터끼리 이벤트 핸들러를 등록한다
...
1. 파티클 업데이트 - **Generate Location Event 모듈 등록**
2. 이벤트 핸들러 스테이지 - **소스에 등록한 이벤트 설정**
3. 이벤트를 받을 이미터의 이벤트 핸들러 - **receive location event 모듈 추가**
    - 통신할 이미터 파티클의 위치 받을 수 있음

> 폭죽 터지듯이 하나의 파티클이 죽으면 다른 파티클이 생성되서 터진다거나 하는 연쇄 이벤트 가능
