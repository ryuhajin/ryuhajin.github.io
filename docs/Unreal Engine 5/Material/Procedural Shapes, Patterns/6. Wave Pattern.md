...
```
1. 가장 먼저 만들 모듈: MF_LineMaskFromDist
    - 거리 → 라인 변환
- 입력
    - Dist (float) // 이미 abs 된 값
    - Thickness (float)
    - Softness (float)
...
내부 로직
- LineMask = 1 - SmoothStep(Thickness, Thickness + Softness, Dist)
...
pCycle (float) // frac 기반 파형용

pRad (float) // sin/cos용 (=pCycle*2π)

내부 로직

pCycle = U*Frequency + Time*Speed + Phase

pRad = pCycle * 6.283185
...
UV (float2) ← 일단은 메시 UV(TexCoord)로 시작
...
pRad = MF_WaveCycle(u, ...)에서 가져옴

waveValue = sin(pRad)

waveY = CenterY + Amplitude * waveValue

dist = abs(v - waveY)

LineMask = MF_LineMaskFromDist(dist, Thickness, Softness)
...
```
1) 그 다음: 월드 스페이스 대응 모듈 MF_ProjectToWaveSpace

이제 UV 대신 월드 공간에서 u/v를 만들 겁니다.
...
u = dot(WorldPos, normalize(Dir)) / WorldScale

v = dot(WorldPos, normalize(Up)) / WorldScale
```

```
그리드 형태 반복 추가: MF_RepeatDistance1D 
...
dNorm = dSigned / Spacing

dWrapped = frac(dNorm + 0.5) - 0.5

distRepeat = abs(dWrapped) * Spacing

그 다음은 그대로:

LineMask = MF_LineMaskFromDist(distRepeat, Thickness, Softness)
...
Wave1, Wave2, Wave3… 각각 mask를 만들고

합성:

max(mask1, mask2, mask3) 추천 (라인 두께 과증가 방지)

saturate(mask1+mask2+...)는 빛이 누적되는 느낌
```

---


현재 내 mf_getPositionSpace
...
2. NormalWS (V3)
    - 기본값 : PixelNormalWS
3. UseLocalSpace? (static bool)
    - 스위치 두 개의 value로 쓰임
    - switch 1 : position 출력용 (true = local position, false = world position)
    - switch 2 : normal 출력용 (trye = local normal, false = world normal)
4. SpaceScale (scalar)
    - 스케일 보정용 값
5. OriginSpace (V3)
   - 기본값 (0,0,0)
   - World, Local 둘 다 쓰임 (OriginWS, OriginLS)
   - world space 일 때 = 월드 기준 원점
   - Local space 일 때 = 오브젝트 원점 (오브젝트 pivot)
6. LocalOffset (V3)
    - local space를 사용할 때 오브젝트 로컬 오프셋 사용가능
...
## logic
1. WorldPos = worldposition - originSpace / Scale
2. LocalPos =
    worldPosLocal = TransformPosition(abs World space -> Local Space, WorldPos),
    OriginLS = OriginSpace (기본값 0,0,0)
    (worldPoslocal - OriginLs - LocalOffset) / scale
3. WorldNormal = normalize(NormalWS)
4. LocalNormal = TransformVector(worldspace -> Local space, worldNormal)
5. UseLocalSpace 를 value로 하는 switch 2개 결과 각각 output에 연결
    - switch 1 (true = localPos, false = worldPos, value = UseLocalSpace) -> output position
    - switch 2 (true = LocalNormal, false = WorldNormal, value = UseLocalSpace) -> output normal
