...
- 기본 불리언 연산:
  - `merge(a,b) = min(a,b)`  (Union)
  - `intersect(a,b) = max(a,b)` (Intersection)
  - `subtract(base, sub) = intersect(base, -sub)` (A ∩ not B)

```c++
float merge(float a, float b)       { return min(a, b); }
float intersect(float a, float b)  { return max(a, b); }
float subtract(float base, float sub) { return intersect(base, -sub); }
```
...
```c++
// 합집합은 단순히 더했기 때문에 새 선분의 부호 거리는 잘못된 스케일을 가짐
// 하지만 단위 정사각형의 대각선인 √2로 나누어 수정할 수 있음
// √2 나누기 = √0.5를 곱하는 것과 동일
// 따라서 sqrt(0.5) 상수 정의

float chamfer_merge(float shape1, float shape2, float chamferSize){
    constexpr float SQRT_05 = 0.70710678118f; // 1/sqrt(2)

    // 일반 합집합 min(a,b)
    float simpleMerge = merge(shape1, shape2);

    // chamfer 후보 계산 (두 거리의 접합점 근처에 대각선 경계 계산. sqrt 적용)
    float chamfer = (shape1 + shape2) * SQRT_05;

    // chamfer를 바깥으로 내기 위해 오프셋
    chamfer = chamfer - chamferSize;

    // 마지막 합집합. 일반 합집합과 챔퍼 결합 min(a,b)
    return merge(simpleMerge, chamfer);
}
```

---

## Chamfer intersection (챔퍼 교집합)

```c++
float chamfer_intersect(float shape1, float shape2, float chamferSize){
    constexpr float SQRT_05 = 0.70710678118f; // 1/sqrt(2)
    ...
    // 교집합은 도형이 줄어드는 방향이 필요하므로 합집합과 오프셋 방향 반대 (내부로 내기 위해 오프셋) 
    chamfer = chamfer + chamferSize;

    // 마지막 교집합. 일반 교집합과 챔퍼 결합 max(a,b)
    return intersect(simpleIntersect, chamfer);
}
```

---

## Chamfer Subtraction (챔퍼 차집합)
차집합 패턴 확장
- `A - B = A ∩ (not B)` 이므로, 두 번째 shape를 부호반전하여 intersection 계열로 처리

```c++
float chamfer_subtract(float base, float subtraction, float chamferSize){
    return chamfer_intersect(base, -subtraction, chamferSize);
}
```

---

# Border Intersection Circle (원형 교차점)
앞에서는 boolean 기반 조합(및 변형)을 다뤘고, 여기서는 **‘두 경계의 교차’를 이용해 새 SDF를 만든다**

---

## 목표
두 도형의 **경계(0)** 가 만나는 위치를 **중심 개념으로 사용해, 그 주변에 원형/라운드 형태의 새 SDF 만들기**

---

## 핵심 아이디어
- 경계 교차점에서는 `shape1 = 0`, `shape2 = 0`
- 따라서 `(shape1, shape2)`를 좌표로 보고 `length((shape1, shape2))`를 구하면 교차점에서 0이 됨
- 원 SDF처럼 반경을 적용: `length(...) - radius`
...
> round_border 자체가 하나의 SDF이므로, 이후 `merge/intersect/subtract`로 다른 도형과 다시 조합할 수 있다

---

# Border Groove
도형의 경계 위치에 다른 도형의 홈 만들기

---

## 목표
- `base` 도형 표면 위에, `groove` 도형의 경계를 따라 **띠 형태의 홈**을 만들고,
- 홈의 **폭(width)** 과 **깊이(depth)** 를 제어
...
1. **abs(groove)**
    - 내부/외부가 모두 안쪽처럼 취급되어 **경계(0)** 를 중심으로 대칭적인 띠를 만들 수 있음
2. **-width**
    - 띠의 두께 (폭) 만들기
...
1. **base + depth**
    - 기본 base를 **inward offset(수축)** 한 형태로 생각할 수 있음
2. **subtract(circleBorder, base + depth)**
    - 경계 band에서 수축된 base를 빼면
    - 홈이 **depth 만큼만** 들어가도록 제한된 groove volume이 만들어짐

---

### 최종 적용
만들어진 groove volume을 base에서 빼서 실제 홈이 파인 결과 만들기
...
float groove_border(float base, float groove, float width, float depth){
    // 1) groove의 경계를 중심으로 띠(band) 생성
    float circleBorder = abs(groove) - width;

    // 2) base를 depth만큼 수축한 형태로 깊이 제한
    float grooveShape = subtract(circleBorder, base + depth);
