...
## Material Capture 개념
MatCap은 실제 반사광 계산을 하지않음!

1. **카메라 뷰 기준**으로 고정된 구형 텍스처(sphere map)를 오브젝트 **표면에 투영**
2. 그 때문에 마치 환경 반사가 있는 것처럼 보이게 만든다

- Material Capture → 반사광과 조명 환경을 캡처한 구형 이미지를 사용
- **오브젝트 표면의 노멀 방향과 카메라 방향을 이용해 텍스처의 어떤 부분을 보여줄지 결정**함
...
| 텍스처    | 구형(spherical) 이미지 |

---

## MatCap vs 정반사광

| 항목 | 정반사광(Phong 등) | MatCap |
| --- | --- | --- |
| 입사 벡터 (L) | 있음 (광원 필요)    | 없음 |
| 기준선  | 법선 (N) | 법선 (N)  |
| 반사 벡터 (R)   | 계산함   | 계산하지 않음 |
| 결과 색  | 실시간 반사광 계산    | 미리 그려진 반사 결과 텍스처에서 샘플링 |
| 뷰 변화에 따른 반응 | 물리적으로 변함  | 텍스처 좌표만 재계산 |

> MatCap은 광원 → 반사를 계산하는 게 아니라, 카메라와 법선의 상대적인 방향만으로 텍스처에서 색을 뽑는다
> - 반사각이라는 개념이 직접적으로 존재하지 않음. 대신 텍스처가 반사각을 대신 표현
