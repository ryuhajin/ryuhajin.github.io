...
### SkyAtmosphereImage
하늘에 지정한 방향으로 텍스쳐를 투영해 주는 노드

```
1. SkySphere를 단위 구(1m짜리 구)라고 생각
2. direction을 이 skysphere 위의 “중심 방향”으로 사용
3. 그 주변의 작은 각도 영역을, 입력 TextureObject의 UV에 매핑
4. 그 결과 색을 현재 픽셀의 뷰 방향에 맞춰 샘플해서 반환
```

### 입력
1. TextureObject (T2D)
    - 타입: Texture Object
    - 하늘에 띄울 이미지 (달 텍스처, 태양 텍스처 등)
2. Direction (float3)
    - 타입: float3 (벡터)
    - 하늘 구면상에서 스프라이트 중심 방향
    - SkyAtmosphereLightDirection 결과를 넣어서, 라이트 방향을 따라 다니게 만든다
3. scale (float2)
    - 이미지 스케일
4. rotation(float)
    - 라디안 각도
...
### 출력
1. Result (float3)
    - SkyAtmosphereImage가 sky 상에서 해당 픽셀 방향에 대해 샘플한 텍스처 색
2. alpha
    - 원본 텍스쳐 오브젝트 알파
    - 투영 때문에 가장자리에서 스트레칭 · 왜곡이 포함될 수 있음
3. mask
    - 노드가 자체적으로 만들어주는 안전 보정 마스크
    - 왜곡 영역 제거
...
## BP와 머티리얼 통신
**블루 프린트는 머티리얼 파라미터에 대해 읽기, 쓰기가 가능**하지만 반대는 불가능 (단방향 통신)

- 머티리얼 그래프에서는 블루프린트 변수 조절 x

### 통신 방법
1. dynamic Material instance
...
### 머티리얼
1. if문 노드 추가
2. bp에서 값을 변경할 `scalar` 파라미터 노드 추가
3. 2번 스칼라 값 (1.0) 을 if문 a에 연결, b는 0
4. `A > B` 일 경우 별 텍스쳐 add
5. `A <= B` 일 경우 별 텍스쳐 not add

---

### InitSkySphereMaterial
블루 프린트에서 다이나믹 머티리얼 사용 전에 init하기

1. InitSkySphereMaterial 함수추가
2. 해당 함수의 리턴 값 bool 타입으로 설정
3. create Dynamic Material instance -> set -> isVaild 값 리턴
4. construction script 시퀀스 실행 전에 if (init 함수)로 vaild 확인
